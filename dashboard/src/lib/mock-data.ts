...
export const MOCK_POIS: PoiRead[] = Object.entries(SAMPLE_WILAYA_SITES).flatMap(
  ([wilayaId, sites]) =>
    sites.map((site) => ({
      ...
      photo_urls: [
        site.photo,
        `https://picsum.photos/seed/${slugify(site.name)}-2/800/500`,
        `https://picsum.photos/seed/${slugify(site.name)}-3/800/500`,
      ],
      ...
      entry_fee_dzd: site.entry_fee_dzd ?? null,
      suggested_duration_min: site.suggested_duration_min ?? null,
